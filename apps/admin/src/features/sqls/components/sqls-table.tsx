...
import { createSqlsColumns } from './sqls-columns'
...
export function SqlsTable({ search, navigate }: SqlsTableProps) {
  ...
  const connectionId =
    typeof search.connection_id === 'string' && search.connection_id.trim()
      ...
      : undefined
  const sqlType =
    typeof search.sql_type === 'string' &&
    SQL_TYPES.includes(search.sql_type as SqlType)
      ? (search.sql_type as SqlType)
      : undefined
  ...
  const columns = useMemo(
    () => createSqlsColumns({ appNameById, connectionNameById }),
          ...
          onValueChange={(value) =>
            navigate({
              search: (prev) => ({
                ...prev,
                page: 1,
                sql_type: value === 'all' ? undefined : value,
              }),
            })
          }
        >
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                ...
                >
                  {(error as Error)?.message || 'Failed to load SQL APIs.'}
                </TableCell>
              </TableRow>
